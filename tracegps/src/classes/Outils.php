...
// estUnNumTelValide($numTelAvalider) : fournit true scorrigerPrenomi le n° de téléphone reçu en paramètre est correct
...
class Outils
{
    ...
    // par exemmple, le paramètre '2007-05-16' donnera '16/05/2007'
    // Dernière mise à jour : 13/7/2021 par dPlanchet
    public static function convertirEnDateFR($laDate): string
    {
    ...
    // Dernière mise à jour : 13/7/2021 par dPlanchet
    public static function convertirEnDateUS($laDate): string
    {
        $tableau = explode("/", $laDate); // Correct
        // L'ordre des segments est incorrect, $jour, $mois et $an sont mal assignés
        $jour = $tableau[0];
        $mois = $tableau[1];
        $an = $tableau[2];
    ...
    public static function corrigerPrenom($lePrenom)
    {
        if ($lePrenom != "") {
            ...
            if ($position === false) {
                ...
                $partie2 = substr($lePrenom, 1, $position - 1); // Correction
    ...
    public static function corrigerTelephone($leNumero)
    {
        ...
        if (strlen($temporaire) == 10)        // il ne doit rester que les 10 chiffres...
        {
            ...
            return $resultat . substr($temporaire, 8, 2);
        } else {
            return $leNumero;
        }
    }
...
// 	}
    ...
    public static function estUnCodePostalValide($codePostalAvalider): bool
    {    // utilisation d'une expression régulière pour vérifier un code postal :
        ...
        if (preg_match($EXPRESSION, $codePostalAvalider) === 1 || $codePostalAvalider == "") return true;
        else return false;
    }
    ...
    // Dernière mise à jour : 11/12/2017 par dP
    public static function estUneAdrMailValide($adrMailAvalider): bool
    {
        ...
        if (preg_match($EXPRESSION, $adrMailAvalider) === 1 || $adrMailAvalider == "") {
            return true;
        } else {
            return false;
        }
    ...
    // (format jj/mm/aaaa ou bien jj-mm-aaaa), false sinon
    // Dernière mise à jour : 11/12/2017 par dP
    public static function estUneDateValide($laDateAvalider): bool
    {    // on retourne true si la date est vide :
        ...
        // test des valeurs
        if ($mois <= 0 || $mois > 12 || $jour <= 0 || $jour > 31) return false;

        else {   // cas des mois de 30 jours
            if (($mois == 4 || $mois == 6 || $mois == 9 || $mois == 11) && ($jour > 30))
                return false;
            else {   // cas du mois de février
    ...
    // (5 groupes de 2 chiffres éventuellement séparés), false sinon
    // Dernière mise à jour : 11/12/2017 par dP
    public static function estUnNumTelValide($numTelAvalider): bool
